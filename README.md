# Datendashboard - Frankfurt - App

This is the Datendashboard Frankfurt. It provides real time information about the climate footprint for the city of [Frankfurt](https://en.wikipedia.org/wiki/Frankfurt). The application displays the data using different charts and a tile-based layout with separate pages for each category. The data is sourced from various providers, including the the German Weather Service (DWD), BrightSky, and other data providers.
...
> > > git clone https://github.com/your-username/sdd-Frankfurt-app.git
> > > cd sdd-Frankfurt-app
...
- **Data visualization:** The application visualizes open smart city data for the city of Frankfurt using different charts, including bar charts, line charts, and scatter plots. The charts are interactive and allow users to filter the data by different criteria.
...
- **Real-time data:** The data in the application is updated in real-time, providing users with the latest information on climate and environmental indicators in the city of Frankfurt.

- **Responsive design:** The application is designed to be responsive, adapting to different screen sizes and devices.

- **Data sources:** The data in the application is sourced from various providers, including the Open Data Portal of the City of Frankfurt, the German Weather Service (DWD), BrightSky, and other data providers.
...
Most of the icons are not included because of licensing restrictions and are part of a private package (@schleegleixner/react-adobe-icons-package). You can either aquire the licences or replace them with an icon pack of your choice.
